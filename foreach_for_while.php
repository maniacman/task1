...
Задачи
10) Дан массив с элементами 2, 5, 9, 15, 0, 4. С помощью цикла foreach и оператора if выведите на экран столбец тех 
элементов массива, которые больше 3-х, но меньше 10.
$arr = [2, 5, 9, 15, 0, 4];
foreach ($arr as $elem) {
	if ($elem > 3 && $elem <= 10) echo $elem . '<br>';
}
...
foreach ($arr as $key => $elem) {
	if ($key < 6) echo $elem . '<br>'; else echo '<b>' . $elem  . '</b>' . '<br>';
}
...
$arr = [1=>'Понедельник', 2=>'Вторник', 3=>'Среда', 4=>'Четверг', 5=>'Пятница', 6=>'Суббота', 7=>'Воскресенье'];
foreach ($arr as $key => $elem) {
	if ($elem != $day) echo $elem . '<br>'; else echo '<b>' . $elem  . '</b>' . '<br>';
}
...
19) Дано число $num=1000. Делите его на 2 столько раз, пока результат деления не станет меньше 50. 
